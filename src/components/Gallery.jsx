...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="py-16 sm:py-20 bg-dark-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-8 sm:mb-10">
          <p className="text-brand-400 text-xs sm:text-sm font-semibold tracking-widest uppercase mb-2 sm:mb-3">Our Fleet</p>
          <h2 className="font-display font-bold text-2xl sm:text-3xl lg:text-4xl text-white mb-3 sm:mb-4">Gallery</h2>
          <p className="text-gray-400 text-sm sm:text-base max-w-lg mx-auto px-4">Real photos of our vehicles - what you see is what you get.</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8 sm:mb-10">
          {cats.map(c => (
            <button key={c} onClick={() => setFilter(c)}
              className={`px-4 sm:px-5 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 ${
                filter === c
          ...
          {filtered.map((img, i) => (
            <button key={i} onClick={() => setLightbox(img)}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                <p className="text-white text-[10px] sm:text-xs font-medium">{img.label}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {lightbox && (
        <div
          ...
          <button onClick={e => { e.stopPropagation(); prev() }}
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-dark-700/80 hover:bg-dark-600 rounded-full flex items-center justify-center text-white transition-colors">
            <ChevronLeft size={16} className="sm:text-[20px]" />
          </button>
          ...
          <div className="relative max-w-4xl max-h-[85vh] w-full" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-center text-white text-xs sm:text-sm mt-2 sm:mt-3 font-medium">{lightbox.label}</p>
          </div>

          <button onClick={e => { e.stopPropagation(); next() }}
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-dark-700/80 hover:bg-dark-600 rounded-full flex items-center justify-center text-white transition-colors">
            <ChevronRight size={16} className="sm:text-[20px]" />
          </button>

          <button onClick={() => setLightbox(null)}
            className="absolute top-2 sm:top-4 right-2 sm:right-4 w-8 h-8 sm:w-9 sm:h-9 bg-dark-700/80 hover:bg-dark-600 rounded-full flex items-center justify-center text-white transition-colors">
            <X size={16} className="sm:text-[18px]" />
          </button>
        </div>
      )}
    </section>
